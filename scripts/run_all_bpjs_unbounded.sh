...
"ttt 2 2 true false" "ttt 3 3 true false" "ttt 4 4 true false"
)
...
for option in "${options[@]}"; do
  ...
  for i in {1..5}
  do
    timeout 30m /usr/bin/time -a -o run_all_bpjs_unbounded_output.csv -f "$option,$i,%E,%M" mvn exec:java -D"exec.args"="$option"
    EXIT_STATUS=$?

...
"ttt 2 2 true true" "ttt 3 3 true true" "ttt 4 4 true true"
)
...
for option in "${options[@]}"; do
  ...
  for i in {1..5}
  do
    timeout 30m /usr/bin/time -a -o run_all_bpjs_bounded_output.csv -f "$option,$i,%E,%M" mvn exec:java -D"exec.args"="$option"
    EXIT_STATUS=$?

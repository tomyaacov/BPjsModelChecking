...
"dining_philosophers 3 true true" "dining_philosophers 6 true true" "dining_philosophers 9 true true" "dining_philosophers 12 true true" "dining_philosophers 15 true true"

"ttt 2 2 true true" "ttt 3 3 true true" "ttt 4 4 true true" "ttt 5 5 true true"
)

echo "option,time,memory" > run_all_bpjs_bounded_output.csv
for option in "${options[@]}"; do
  echo "$option"
  timeout 30m /usr/bin/time -a -o run_all_bpjs_bounded_output.csv -f "$option,%E,%M" mvn exec:java -D"exec.args"="$option"
    EXIT_STATUS=$?
    if [ $EXIT_STATUS -eq 124 ]
    then
    echo "$option,t.o." >> run_all_bpjs_bounded_output.csv
    else
    if [ $EXIT_STATUS -ne 0 ]
    then
    echo "$option,error,$EXIT_STATUS" >> run_all_bpjs_bounded_output.csv
    fi
    fi
done
